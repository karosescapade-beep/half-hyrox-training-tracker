...
function hasPlannedStrength(workout) {
  return workout.includes("Barry") || workout.includes("F45") || workout.includes("OTF");
}

function hasPlannedFlexibility(workout) {
  return workout.includes("Pilates") || workout.toLowerCase().includes("flexibility");
}

function isClassChoiceDayTask(task) {
  const weekday = task.originalDay?.slice(0, 3);
  if (!["Wed", "Thu", "Fri"].includes(weekday)) return false;
  const theme = getTheme(task.workout);
  return !task.isRun && theme !== "Recovery" && theme !== "Race" && theme !== "Celebrate";
}

function getStrengthChoiceDefault(task, isSundayTraining, isClassChoiceDay) {
  if (isSundayTraining) return "None";
  if (isClassChoiceDay && !hasPlannedStrength(task.workout)) return "None";
  return getStrengthDefault(task.workout);
}

function getFlexibilityChoiceDefault(task, isSundayTraining, isClassChoiceDay) {
  if (isSundayTraining) return "None";
  if (isClassChoiceDay && !hasPlannedFlexibility(task.workout)) return "None";
  return "Pilates";
}
...
function renderWorkout(task) {
  ...
  const isClassChoiceDay = isClassChoiceDayTask(task);
  const hasStrength = isSundayTraining || isClassChoiceDay || hasPlannedStrength(workout);
  const hasFlexibility = isSundayTraining || isClassChoiceDay || hasPlannedFlexibility(workout);
  const theme = getTheme(workout);
  const hasRun = task.isRun || workout.includes("miles");
  const canSkipCategory = isSundayTraining || isClassChoiceDay || (hasStrength && hasFlexibility) || (hasRun && hasFlexibility);
  const strengthDefault = getStrengthChoiceDefault(task, isSundayTraining, isClassChoiceDay);
  const flexibilityDefault = getFlexibilityChoiceDefault(task, isSundayTraining, isClassChoiceDay);
  ...
  if (hasStrength) {
    const strengthOptions = canSkipCategory ? ["Barry's", "F45", "OTF", "None"] : ["Barry's", "F45", "OTF"];
    const strength = makeChoiceSelect(id, "Strength", strengthOptions, strengthDefault);
    fragment.append(makeThemeLine("Strength", strength));
  }

  if (hasFlexibility) {
    const flexibilityOptions = canSkipCategory
      ? ["Pilates", "Barre", "Yoga", "None"]
      : ["Pilates", "Barre", "Yoga"];
    const flexibility = makeChoiceSelect(id, "Flexibility", flexibilityOptions, flexibilityDefault);
    fragment.append(makeThemeLine("Flexibility", flexibility));
  }
...
function getTaskWorkoutCredit(task, profile) {
  ...
  if (hasRun) credit += 1;
  if (hasStrength) credit += getChoiceWorkoutCredit(task.id, "Strength", strengthDefault, profile);
  if (hasFlexibility) credit += getChoiceWorkoutCredit(task.id, "Flexibility", flexibilityDefault, profile);
